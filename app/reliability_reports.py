...
def finish_reliability_run(run_id: int, player_id: str, scenario: str) -> dict[str, object]:
    ...
    try:
        ...
        if scenario == "guard_disabled_control":
            ...
            record_reliability_event(run_id, "detection", "已检出账本守卫失效导致的重复余额副作用" if verification_passed else "对照失效未被检出", **assertion)
        else:
            verification_passed = assertion["passed"]
            status = "passed" if verification_passed else "failed"
            record_reliability_event(run_id, "assertion", "最终不变量校验通过" if verification_passed else "最终不变量校验失败", **assertion)
        error_message = None
    ...
    with connect() as connection:
        connection.execute(
            ...
            {"status": status, "completed_at": utc_now(), "passed": int(verification_passed), "summary_json": json.dumps({"verification_passed": verification_passed, "invariant_passed": assertion["passed"], **assertion}, ensure_ascii=False), "error_message": error_message, "run_id": run_id},
        )
    ...
    with connect() as connection:
        connection.execute(
            text("""UPDATE reliability_runs SET status = 'failed', completed_at = :completed_at, passed = 0,
                summary_json = :summary_json, error_message = :error_message WHERE run_id = :run_id"""),
            {"completed_at": utc_now(), "summary_json": json.dumps({"verification_passed": False, "invariant_passed": False, "passed": False, "expected": {}, "actual": {}}, ensure_ascii=False), "error_message": message, "run_id": run_id},
        )
    return get_reliability_run(run_id) or {}
